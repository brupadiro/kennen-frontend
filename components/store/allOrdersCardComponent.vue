<template>
  <GeneralCardComponent class="mx-auto">
      ...
      <v-row>
        <v-col class="col-md-11 col-12">
          <v-row>
            ...
            <v-col cols="12" md="6">
              <FormsFieldsTextComponent v-model="search.startDate" type="date" placeholder="Fecha inicio"></FormsFieldsTextComponent>
            </v-col>
            <v-col cols="12" md="6">
              <FormsFieldsTextComponent v-model="search.endDate" type="date" placeholder="Fecha fin"></FormsFieldsTextComponent>
            </v-col>
            <v-col cols="12" md="12">
    ...
    <v-card-title>
      <v-card outlined>
      <v-row no-gutters>
          <v-switch v-model="col.visible" :label="col.text" class="mr-2 my-2" v-for="col in columns" :key="col.value"></v-switch>
      </v-row>
    </v-card>
    ...
    <v-card-text>
      <v-data-table item-key="id" show-expand single-expand :loading="loading" loading-text="Cargando ordenes"
        ...
        <template v-slot:item.product.tracking_number="{ item }">
          ...
          <v-edit-dialog 
          :return-value.sync="item.product.tracking_number"
          save-text="Guardar"
          cancel-text="Guardar"
           
          @save="updateTrackingOrder(item)"
        >
          <v-text-field outlined dense height="20"  hide-details readonly :value="setValue('#',item.product.tracking_number)"></v-text-field>
          <template v-slot:input>
            <v-text-field
              v-model="item.product.tracking_number"
              label="Edit"
              @keydown.enter="updateTrackingOrder(item)"
            ></v-text-field>
          </template>
        </v-edit-dialog>
        </template>
        ...
        <template v-slot:item.product.net_price="{ item }">
          <v-edit-dialog 
          :return-value.sync="item.product.net_price"
        >
          <v-text-field outlined dense height="20" hide-details readonly :value="setValue('$',item.product.net_price)"></v-text-field>
          <template v-slot:input>
            <v-text-field
              v-model="item.product.net_price"
              @keydown.enter="updateTrackingOrder(item)"
              label="Edit"
              counter
            ></v-text-field>
              ...
              <v-card-title>
                <v-row>
                  ...
                  <v-col class="col-md-8 col-12">
                    <FormsFieldsTextButtonComponent dense notification-text="Incidencia agregada" @click="addIncidences($event,item)" label="Agregar incidencia"></FormsFieldsTextButtonComponent>
                  </v-col>
                </v-row>
              </v-card-title>
              <v-divider></v-divider>
